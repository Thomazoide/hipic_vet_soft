...
export default function AdminPreps({query}){
    ...
    const [verPrepData, setVerPrepData] = useState(false)
    const [verVetData, setVerVetData] = useState(false)
    const [verHorseData, setVerHorseData] = useState(false)
    ...
    const toggleVerPrepData = () => setVerPrepData(!verPrepData)
    const toggleVerVetData = () => setVerVetData(!verVetData)
    const toggleVerHorseData = () => setVerHorseData(!verHorseData)
            ...
            <div className='ficha-info'>
                {
                    selectedPrep && equipos ? <>
                        ...
                        { selectedPrep.tipo === 'team' ? <div className='lista-preps'>
                            <h1> {selectedPrep.codigo} </h1>
                            <hr/>
                            <h4>Preparador:</h4>
                            <p><strong> {selectedPrep.prep[0].nombre} </strong> </p>
                            <p><Button variant='outline-success' onClick={toggleVerPrepData} size='sm' > {!verPrepData ? 'Ver datos' : 'Ocultar datos'} </Button> </p>
                            { verPrepData ? <>
                            <hr/>
                                ...
                                !selectedPrep.vets[0] ? <> <p className='text-warning'>Sin veterinarios registrados</p> <hr/> </> : <>
                                    {
                                        selectedPrep.vets.map( vet => <Container key={vet.rut} >
                                            <p> Veterinario/a: <strong> {vet.nombre} </strong> </p>
                                            <p> <Button variant='outline-success' size='sm' onClick={toggleVerVetData} > {!verVetData ? 'Ver datos' : 'Ocultar datos'} </Button> </p>
                                            {verVetData ? <>
                                            <p> Rut: {vet.rut} </p>
                                            <p> Email: {vet.email} </p>
                                ...
                                !selectedPrep.horses[0] ? <p className='text-warning'>Sin caballos registrados</p> : <>
                                    {
                                        selectedPrep.horses.map( h => <Container key={h.codigo_caballo} >
                                            <p>Nombre: <strong> {h.nombre} </strong> </p>
                                            <p> <Button variant='outline-success' size='sm' onClick={toggleVerHorseData}> {!verHorseData ? 'Ver datos' : 'Ocultar datos'} </Button> </p>
                                            <hr/>
                                            {verHorseData ? <>
    ...
    return(
        <div className='preps-info'>
            ...
            <div className='enrolador'>
                { equiposDisp && queryData && corralesDisponibles ? <Container className='lista-crear-preps'>
                <h1>Crear usuario</h1>
